...
/***********************************************************/
/*Note: for this program, adherence = 0 for missed visits***/
/*Estiamtes % difference and 95%CI via bootstrapping********/
/***********************************************************/
...
%macro partA_adjusted(nboot = , outdest = , inset=, titlemain =, lib=, adhvar = );
...
%do bsample =  0 %to &nboot ;   
	...
	%if %eval(&bsample) = 0 %then %do;
		proc reg data = onesample plots=none outest = est0;
			model dth5 = &adhvar 
			age_bin nonwhite IRK MI_bin NIHA_bin1 RBW_bin
			CHF ACI AP IC ICIA DIG DIUR AntiArr AntiHyp OralHyp 
			CardioM AnyQQS AnySTDep AnyTWave STElev FVEB VCD 
			HiHeart HiDiasBP HiSySBP HiBili HiSerChol HiSerTrigly HiSerUric HiSerAlk 
			HiPlasUrea HiFastGluc HiOneGluc HiWhiteCell HiNeut HiHemat Cig Inact  ;
...
%partA_unadjusted( outdest = "PartA.Unadj.final.rtf", inset = binary, titlemain = "Unadjusted, 1980 Replication, Missed Adherence = 0", 
		nboot = &nboot, lib = cdp);

/*original 1980 adherence definition*/
%partA_adjusted( outdest = "PartA.Adj.linear.final.rtf", inset = binary, 
	titlemain = 'Adjusted, 1980 Replication, Missed Adherence = 0',  nboot = &nboot, lib=cdp, adhvar = old_adhx15bin);


/*updated adherence definition*/
%partA_adjusted( outdest = "PartB.Adj.linear.final.rtf", inset = binary, 
	titlemain = 'Adjusted, 1980 Replication, Missed Adherence = 0',  nboot = &nboot, lib=cdp, adhvar = adhx15bin);
